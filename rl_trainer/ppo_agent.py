...
class PPOAgent:
    def __init__(self, state_dim, action_dim, lr=1e-4, gamma=0.95, eps_clip=0.2, K_epochs=6):
        self.gamma = gamma
...
def train_pong():
    env = PongEnv()
    agent = PPOAgent(state_dim=env.observation_space.shape, action_dim=env.action_space.n)
    ...
    while episode_count < max_episodes:
        ...
        if done or timestep >= max_steps_per_episode:
            episode_rewards.append(episode_reward)
            avg_reward = np.mean(episode_rewards[-100:])
            avg_rewards.append(avg_reward)

            print(f"Episode: {episode_count+1}, Reward: {episode_reward:.2f}, Avg (100): {avg_reward:.2f}")
            ...
            if episode_count % 50 == 0:
                torch.save(agent.policy.state_dict(), f"pong_policy_episode_{episode_count}.pth")
